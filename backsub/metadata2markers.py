...
def meta_from_file(src_img_path, registration_filter):
    ...
    ch_names = make_marker_names_unique_list( [ element.name for element in ome.images[0].pixels.channels ] )
    ...
    if registration_filter not in filters:
        raise ValueError(f"Registration filter '{registration_filter}' not found in image metadata filters: {list(set(filters))}")

    background = [None if registration_filter in element
...
def assign_background(df, rmv_ref=False, rmv_back=False, ref_marker="DAPI", registration_filter="DAPI"):
    ...
    if rmv_ref or rmv_back:
        ...
        if rmv_back:
            df.loc[ df["background"].isnull() & (df["Filter"] != registration_filter) , ["remove"] ] = "TRUE"
        ...
        if rmv_ref:
            ...
            if ref_marker not in df.marker_name.values:
                ...
                warnings.warn(f"Reference marker '{ref_marker}' not found. Using first instance of registration filter '{registration_filter}' - Channel '{corr_ref_marker}' instead.")
            else:
...
def main():
    ...
    df_updated = assign_background(df, args.remove_background_references, global_ref_marker, registration_filter)
