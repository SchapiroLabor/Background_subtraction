...
def pyramidal_levels(img_arr, sub_levels, dims_schedule=None):
    ...
    if dims_schedule is None:
        ...
        down_factor = np.rint(
            np.mean(
                [
                    dims_schedule[i][0] / dims_schedule[i + 1][0]
    ...
    for dims in dims_schedule:
        ...
        img_aux = resize(
            img_as_float32(result.compute()),
            dims,
            order=1,
            preserve_range=True,
            anti_aliasing=False,
        )
        img_aux = np.rint(rescale_intensity(img_aux, out_range=val_range)).astype(
            ref_dtype
        )
        yield img_aux
...
def pyramid_save_ram(img_arr, sub_levels):
    chunksize = (
        2048,
        2048,
    )  # This value was found to be optimal, in terms of balanced RAM and runtime, for uint16 data
    down_factor = 2
    ...
    for _ in range(sub_levels):
        ...
        dim_rescale = da.map_overlap(
            rescale,
            float_half,
            depth=border_overlap,
            boundary="reflect",
            **rescale_args,
        )
...
def process_markers(markers):
    ...
    missing_bgs = [
        bg
        for bg in markers["background"].dropna().unique()
        if bg not in markers["marker_name"].values
    ]
    if missing_bgs:
        raise ValueError(
            f"Background markers not found in marker_name column: {missing_bgs}"
        )
    ...
    for channel in range(len(markers)):
        ...
        if markers.processed[channel]:
            bg_idx = markers.loc[
                markers.marker_name == markers.background[channel], "ind"
            ].tolist()

            if len(bg_idx) > 1:
                print(
                    f"""Warning: Background channel with name {markers.background[channel]} 
                      appears several times in the column "marker_name".  
                      Only the first occurrence will be used for the subtraction."""
                )
            bg_idx = bg_idx[0]
            scaling_factor[channel] = (
                markers.exposure[channel] / markers.exposure[bg_idx]
            )
...
def extract_img_props(img_path, downscale_factor, tile_size, pixel_size=None):
    ...
    with tifff.TiffFile(img_path) as tif:
        ...
        if is_pyramid:
            ...
            subres_dims = [
                tif.series[0].levels[lvl].pages[0].shape for lvl in range(1, pyr_levels)
            ]
        else:
            subres_dims = None
    ...
    extracted_levels = int(
        np.floor(np.log(max_dim / tile_size) / np.log(downscale_factor)) + 1
    )
...
def subtract_channels(
    ...
    signal_as_zarr = zarr.open(
        tifff.imread(
            src_img_path, aszarr=True, series=0, level=0, key=int(signal_index)
        )
    )
    background_as_zarr = zarr.open(
        tifff.imread(
            src_img_path, aszarr=True, series=0, level=0, key=int(background_index)
        )
    )
    ...
    if np.issubdtype(ref_dtype, np.integer):
        ...
        subtraction = da.clip(signal - factor * background, info.min, info.max).astype(
            ref_dtype
        )
    ...
    with tifff.TiffWriter(out_file, bigtiff=True) as tif:
        ...
        for _, channel in tasks_table.iterrows():
            if channel.processed:
                ...
                first_layer = tifff.imread(
                    src_img_path, series=0, level=0, key=int(channel.ind)
                )
            ...
            if pyramid_action == "calculate":
                ...
                pyramid = extract_sublevels_from_tiff(
                    src_img_path, int(channel.ind), levels
                )
...
def main():
    ...
    if args.compression not in ["none", "lzw", "deflate", "zlib"]:
        raise ValueError(
            "Compression has to be one of the following: 'none','lzw','deflate','zlib'"
        )

    tile_shape = (args.tile_size, args.tile_size)

    # 1) Extract image properties
    src_props = extract_img_props(
        in_path, args.downscale_factor, args.tile_size, args.pixel_size
    )
    ...
    markers_preview = markers_updated.drop(
        columns=["keep", "ind", "processed", "factor", "bg_idx"]
    )
