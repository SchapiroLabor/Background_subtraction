...
def write_pyramid(img_instances,
    ...
    with tifff.TiffWriter(out_file_path, ome=False, bigtiff=True) as tif:
        ...
        for img,chann_idx in img_instances:
            #aux_img=img
            #pyramid_levels=pyramid_gaussian( aux_img, max_layer=sublayers, preserve_range=True,order=1,sigma=1)
...
def subtract_channels(img_path, markers_info,ref_dtype):
    ...
    for _,channel in markers_info.iterrows():
        ...
        if channel.processed:
            ...
            """
            yield ( tifff.imread(img_path,series=0,level=0,key=int(channel.ind)),int(channel.ind) )
